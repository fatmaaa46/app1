...
function MenuCategorie({ showModal, setShowModal }: any) {
  ...
  const handleCommandeClick = (item?: string) => {
    setSelectedCategorie(item !== undefined ? item : (document.getElementById("mySelect") as any).value)
            ...
            <div className="mt-2" >
              <ul style={{ backgroundColor: "#FFFFFF", cursor: "pointer" }} >
                {Object.entries(card.categories)
                  .slice(0, 7)
                  .map(([key, value]: any) => (
                    <li key={key} onClick={() => handleCommandeClick(value.title)}>{value.title}</li>
                  ))}
              </ul>
            </div>
            {Object.keys(card.categories).length > 7 && (
              <div className="d-flex align-items-center">
                <select className="max-w-xs nav-link mx-5" id="mySelect" onChange={() => handleCommandeClick()}>
                  {Object.entries(card.categories)
